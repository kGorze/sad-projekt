...
repair_dataset <- function(data, missing_threshold = 0.1, outlier_method = "iqr", 
  ...
  if (any(missing_analysis$missing_percentages > missing_threshold * 100) || 
    ...
    if (missing_threshold == 0 && sum(missing_analysis$missing_counts) > 0) {
      ...
      # Task G: Missing Data Sensitivity Analysis using consolidated function
      cat("=== TASK G: MISSING DATA SENSITIVITY ANALYSIS ===\n")
      cat("Using consolidated function from statistical_helpers.R\n")
      
      # Source the consolidated functions
      source("modules/utils/statistical_helpers.R")
      
      # Get first column with missing data
      missing_columns <- names(which(missing_analysis$missing_counts > 0))
      
      if (length(missing_columns) > 0) {
        # Call consolidated missing data sensitivity analysis
        sensitivity_results <- missing_data_sensitivity(data, missing_columns[1], "grupa", n_imputations = 5)
        repair_log$sensitivity_analysis <- sensitivity_results
        
        # Use simple fallback if sensitivity analysis doesn't provide recommendations
        data <- data  # Keep original data since consolidated function doesn't return recommended_data
        repair_log$missing_treatment <- list(
          method = "consolidated_sensitivity_analysis",
          columns_imputed = missing_columns,
        ...
        sensitivity_results <- list(message = "No missing data", missing_percent = 0)
        repair_log$sensitivity_analysis <- sensitivity_results
        repair_log$missing_treatment <- list(
          method = "none",
          columns_imputed = c(),
